...
module hazard(
    ...
    input wire inst_stall,      // 指令握手等待
    input wire data_stall,      // 数据访存等待
    ...
    always @(*) begin
        ...
        // 如果发生异常，强制不暂停(让PC更新)
        stallF = rst ? 1'b0 : (~flush_exception & (lwstall | branch_stall | jump_stall | link_stall | stall_divE | inst_stall | data_stall));
        stallD = rst ? 1'b0 : (~flush_exception & (lwstall | branch_stall | jump_stall | link_stall | stall_divE | inst_stall | data_stall));
        
        // E 阶段暂停：仅在除法运算时暂停，保持 ALU 状态
        stallE = rst ? 1'b0 : (~flush_exception & (stall_divE | data_stall)); 
        
        // W 阶段暂停：等待数据返回，保持写回一致
        stallW = rst ? 1'b0 : (~flush_exception & data_stall);
