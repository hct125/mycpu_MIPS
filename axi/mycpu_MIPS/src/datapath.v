...
module datapath(
    ...
    output wire stallD,stallE,stallW,
    output wire flushD,flushE,flushM,flushW,
    output wire validM_out,
    ...
    wire inst_stall;
    wire data_stall;
    assign inst_stall = ~inst_data_ok;
    assign data_stall = memtoregM & ~data_data_ok;
    ...
    flopenrc #(1) r_validD(.clk(clk),.rst(rst),.en(~stallD),.clear(flushD),.d(inst_data_ok),.q(validD));
    ...
    flopenrc #(1) r_validE(.clk(clk),.rst(rst),.en(~stallE),.clear(flushE),.d(validD),.q(validE));
    ...
    assign data_size = (opM == `LB || opM == `LBU || opM == `SB) ? 2'b00 :
                       (opM == `LH || opM == `LHU || opM == `SH) ? 2'b01 :
                       2'b10;
    ...
    flopenrc #(1) r_validM(.clk(clk),.rst(rst),.en(1'b1),.clear(flushM),.d(validE),.q(validM));
    assign validM_out = validM;
    ...
    flopenrc #(1) r_validW(.clk(clk),.rst(rst),.en(~stallW),.clear(flushW),.d(validM),.q(validW));
    ...
    assign debug_wb_rf_wen = {4{regwrite & validW & (pcW != pcW_prev)}};  // 4位写使能信号
