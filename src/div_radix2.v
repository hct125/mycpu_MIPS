...
1.2: 增加 ready 信号，适配 datapath 握手逻辑; 修复背靠背 bug
*/
//学长版做参考
`timescale 1ns/1ps
...
module div_radix2(
    ...
    output wire         ready,  // High when result is valid
    output [63:0]       result
    );
    /*
    ...
    */
    ...
    reg [5:0] cnt;
    reg start_cnt;
    reg done_wait; // 增加等待信号, 同时作为 ready 信号
    ...
    always @(posedge clk) begin
        ...
        // 增加 !done_wait 条件，防止在刚结束的周期立刻重启
        else if(!start_cnt & valid & !done_wait) begin
            cnt <= 1;
            start_cnt <= 1;
            //save a,b
            a_save <= a;
        ...
        else if(done_wait) begin
            done_wait <= 0; // 冷却一拍，准备好接收新指令
        end
    end
    
    assign ready = done_wait;
endmodule